			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="specifics" title="${message(code: 'address.specifics.label', default: 'Specifics')}" />
					
						<g:sortableColumn property="state" title="${message(code: 'address.state.label', default: 'State')}" />
					
						<g:sortableColumn property="streetAddr" title="${message(code: 'address.streetAddr.label', default: 'Street Addr')}" />
				...
				<g:each in="${addressInstanceList}" status="i" var="addressInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: addressInstance, field: "specifics")}</td>
					
						<td>${fieldValue(bean: addressInstance, field: "state")}</td>
					
						<td>${fieldValue(bean: addressInstance, field: "streetAddr")}</td>
