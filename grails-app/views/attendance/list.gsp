			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="attendance.member.label" default="Member" /></th>
					
						<th><g:message code="attendance.event.label" default="Event" /></th>
				...
				<g:each in="${attendanceInstanceList}" status="i" var="attendanceInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${attendanceInstance.id}">${fieldValue(bean: attendanceInstance, field: "member")}</g:link></td>
					
						<td>${fieldValue(bean: attendanceInstance, field: "event")}</td>
